...
public class ChatClient02 extends MFrame 
implements ActionListener, Runnable {
	...
	public ChatClient02() {
		...
		p1.add(tf1 = new TextField("192.168.0.15"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj==bt1/*connect*/) {
			...
			String file = "ch15/" + System.currentTimeMillis() + ".txt";
			try {
				FileWriter fw = new FileWriter("ch15/"+file+"");
				fw.write(area.getText());
				fw.flush();
				fw.close();
				new MDialog(this,"save","저장하였습니다.");
			} catch (Exception e2) {
				e2.printStackTrace();
			}
			...
			if(filterMgr(str)) { //public boolean filterMgr(String msg)
				new MDialog(this,"경고", "금지어입니다."); //public MDialog(ChatClient02 ct2,String title, String msg) {
				return;
			}
			if(!flag/*아이디 입력 일때*/) {
				sendMessage(ChatProtocol02.ID+":"+str);
				setTitle(getTitle()+"-"+"님 반갑습니다.");
				area.setText("");				
				flag = true; 
				...
				if(i==-1||i==0) { //전체채팅
					sendMessage(ChatProtocol02.CHATALL+":"+str);
				}else { //귓속말 채팅
					String id = list.getSelectedItem();
					sendMessage(ChatProtocol02.CHAT+";"+str);
				}
				tf3.setText("");
				tf3.requestFocus();
			}
	...
	public void routine(String line) {
		...
		if(cmd.equals(ChatProtocol02.CHATLIST)) {
			//data: aaa;bbb;ccc;
			list.removeAll();
			list.add(listTitle);//대화자명단, index 0번째 값
		...
		}else if(cmd.equals(ChatProtocol02.CHAT)||cmd.equals(ChatProtocol02.CHATALL)){		
			area.append(data+"\n");
			...
			data = data.substring(idx+1); //뭐해?
			new Message("FROM", cmd, data);		//public Message(String mode, String id, String msg)
	...
	class Message extends Frame implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==send){
				sendMessage(ChatProtocol02.MESSAGE+
						":"+id+";"+ ta.getText());
			}
			setVisible(false);
			dispose();
	...
	class MDialog extends Dialog implements ActionListener{
		...
		public MDialog(ChatClient02 ct2,String title, String msg) {
