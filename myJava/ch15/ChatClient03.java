...
public class ChatClient03 extends MFrame implements ActionListener, Runnable {
	...
	public ChatClient03(BufferedReader in,PrintWriter out,String id) {
		setSize(450, 500);
		this.in = in;
		this.out = out;
		this.id = id;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == bt2/* save */) {
			...
			if (i == -1 || i == 0) {
				// 전체채팅
				sendMessage(ChatProtocol03.CHATALL + ":" + str);
			} else {
				// 1:1채팅
				String id = list.getSelectedItem();
				sendMessage(ChatProtocol03.CHAT + ":" + id + ";" + str);
			}
			tf3.setText("");
			tf3.requestFocus();
		}
	...
	public void routine(String line) {
		...
		if (cmd.equals(ChatProtocol03.CHAT)
				|| cmd.equals(ChatProtocol03.CHATALL)) {
			area.append(data + "\n");
		} else if (cmd.equals(ChatProtocol03.CHATLIST)) {
			list.removeAll();
			list.add(listTitle);
			// aaa;bbb;ccc;
...
class MDialog extends Dialog implements ActionListener{
		...
		public MDialog(ChatClient03 ct3,String title, String msg) {
	...
	class Message extends MFrame implements ActionListener {
		...
		public Message(ChatClient03 ct2, String mode) {
			setTitle("쪽지보내기");
			this.ct2 = ct2;
			this.mode = mode;
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == send) {
				ct2.sendMessage(ChatProtocol03.MESSAGE + ":" + id + ";"
						+ ta.getText());
			}
			setVisible(false);
			dispose();
